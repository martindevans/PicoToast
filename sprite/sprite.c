...
void __ram_func(sprite_sprite8)(uint8_t *scanbuf, const sprite_t *sp, uint raster_y, uint raster_w) {
    ...
    intersect_t isct = get_sprite_intersect(sp, raster_y, raster_w);
    if (isct.size_x <= 0)
        return;
...
void __ram_func(sprite_sprite16)(uint16_t *scanbuf, const sprite_t *sp, uint raster_y, uint raster_w)
{
    ...
    intersect_t isct = calculate_sprite_span(sp, raster_y, raster_w, sizeof(uint16_t));
    if (isct.size_x <= 0)
        return;
    
    bool span_continuous = !isct.span_discontinuous;
    if (span_continuous)
